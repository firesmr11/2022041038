# 2022401038 박정환 성적 관리 프로그램 최종최종
...
def score_add(program):
    ...
    for i in range(len(program)):  # 입력된 학생 수만큼 반복
...
def ec(aveg):  # 80점 이상 학생 수 계산 함수수
    count = 0
    for i in range(len(aveg)):  # 학생 수만큼 반복
        if float(aveg[i]) >= 80:
            count += 1
    return count


def pr(program, total, aveg, grad, rank, ecc):  # 출력함수
    text = "성적관리프로그램"
    print(text.center(50))
    print('=' * 80)
    print('{:<15}{:<13}{:<5}{:<5}{:<5}{:<5}{:<5}{:<5}{:<5}'.format('학번', '이름', '영어', 'C-언어', '파이썬', '총점', '평균', '학점',
                                                                   '등수'))
    print('=' * 80)

    n = len(program)  # 입력된 학생 수 기반으로 출력
    for i in range(n):
        print(
            f"{program[i][0]:<15} {program[i][1]:<15} {program[i][2]:<7} {program[i][3]:<7} {program[i][4]:<7} {total[i]:<7}{aveg[i]:<7.2f} {grad[i]:<7} {rank[i]:<7} ")
...
grad = [grade(a) for a in aveg]  # 학점 리스트 생성
rank = rank_md(aveg)  # 등수 계산

program = totalsort(program, total, aveg, grad, rank)  # 총점 정렬 함수 호출
